...
class PharmacyController(http.Controller):
    ...
    def pharmacy_page(self, search="", city_filter="", permanence_filter="", latitude=None, longitude=None,
                      show_more=False):
        ...
        if latitude and longitude:
            ...
            pharmacies_with_distances = [
                (pharmacy, self.haversine(user_location, (pharmacy.latitude, pharmacy.longitude))) for pharmacy in
                pharmacies]
    ...
    @http.route('/pharmacy/detail/<int:pharmacy_id>/<int:medicine_id>/book/<int:bookingNbr>', auth="public",
                website=True)
    def book_pharmacy_detail_page(self, pharmacy_id, medicine_id, bookingNbr):
        ...
        stock_record = request.env['pharmacy.stock'].search(
            [('pharmacie_id.id', '=', pharmacy_id), ('medicament_id.id', '=', medicine_id)])
